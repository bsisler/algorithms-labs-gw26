...
def bst_insert(tree, key):
  ...
  """
  # 2.3A: Traverse downward to find parent slot, attach Node(key, parent=...), and update tree.root if empty.
  z = Node(key)
  parent = None
  current = tree.root
  while current != None:
...
def bst_delete(tree, key):
  ...
  """
  # 2.3B: Find target node z; handle 0-child, 1-child, and 2-child cases using transplant and successor.
